...
ASICDriver::ASICDriver()
{
	...
	program_conf_file = fs::path(sde_install) / "share/p4/targets/tofino/allreduce_poc.conf";
	...
	bf_ret = dev_mgr.bfRtInfoGet(dev_tgt.dev_id, "allreduce_poc", &bfrt_info);
...
void ASICDriver::initial_setup()
{
	...
	/* Create broadcast group for aggregation */
	port_ids.clear();
	for (int i = 0; i < 1; i++)
	{
		for (int j = 0; j < 1; j++)
	...
	check_bf_status(pre_node->tableEntryAdd(*session, dev_tgt,
				*table_create_key<uint64_t>(pre_node, {"$MULTICAST_NODE_ID", 0x10}),
				*table_create_data<uint64_t, vector<bf_rt_id_t>, vector<bf_rt_id_t>>(
					pre_node,
					{"$MULTICAST_RID", 1},
					{"$MULTICAST_LAG_ID", vector<bf_rt_id_t>()},
	...
	check_bf_status(pre_mgid->tableEntryAdd(*session, dev_tgt,
				...
				*table_create_data<vector<bf_rt_id_t>, vector<bool>, vector<bf_rt_id_t>>(
					pre_mgid,
					{"$MULTICAST_NODE_ID", vector<bf_rt_id_t>({0x10})},
					{"$MULTICAST_NODE_L1_XID_VALID", vector<bool>({false})},
					{"$MULTICAST_NODE_L1_XID", vector<bf_rt_id_t>({0})}
				)
			),
	...
	/* Switching table entry for aggregation */
	MacAddr agg_mac;
	memcpy(agg_mac.addr, "\x62\x00\x00\x00\x00\x00", sizeof(agg_mac));

	check_bf_status(table_add_or_mod(*switching_table_src,
				*session, dev_tgt,
				*table_create_key<uint8_t*>(
					switching_table_src,
					{"hdr.ethernet.src_addr", reinterpret_cast<uint8_t*>(&agg_mac), sizeof(agg_mac)}
				),
				*table_create_data_action<>(
					switching_table_src,
					"Ingress.stsrc_static"
				)
			),
	...
	check_bf_status(table_add_or_mod(*switching_table,
				...
				*table_create_key<uint8_t*>(
					switching_table,
					{"hdr.ethernet.dst_addr", reinterpret_cast<uint8_t*>(&agg_mac), sizeof(agg_mac)}
				),
				*table_create_data_action<>(
					switching_table,
					"Ingress.aggregate"
